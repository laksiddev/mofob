...
namespace Open.MOF.Messaging.Adapters
{
    public class WcfClientMessagingAdapter : MessagingAdapter
    {
        ...
        protected internal override bool CanSupportMessage(FrameworkMessage message)
        {
            Initialize();
            ...
            bool messageDefinedInEndpointContract = _endpointParameterLookup.ContainsKey(messageType);

            bool messageHasCompatibleAddress = true;
            if (message.To != null)
            {
                messageHasCompatibleAddress = ((_endpointParameterLookup[messageType].ContainsKey(message.To.Action)) && 
                    (WcfUtility.DoesAddressMatchBinding(message.To.Uri, _bindingType)));
            }
